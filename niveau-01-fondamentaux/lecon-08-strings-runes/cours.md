# Leçon 8 — Strings, runes et bytes
...
// (strings.TrimFunc accepterait une fonction en paramètre — plus souple, mais
// les types fonction ne sont vus qu'à la leçon 11.)
...
func initials(phrase string) string {
	...
	for _, w := range strings.Fields(phrase) {
		...
		if size == 0 || !unicode.IsLetter(r) {
			continue // ignore les jetons qui ne commencent pas par une lettre
		}
...
|---|---|
| `strings.Trim(s, cutset)` | Retire en tête et en fin **tout caractère appartenant à l'ensemble** — ce n'est pas un préfixe à retirer, piège classique. La variante `TrimFunc`, plus souple, prend une fonction : leçon 11. |
...
| `size == 0 \|\| !unicode.IsLetter(r)` | Écarte la chaîne vide **et** les jetons de ponctuation isolés : `strings.Fields` considère `!` comme un mot. |
