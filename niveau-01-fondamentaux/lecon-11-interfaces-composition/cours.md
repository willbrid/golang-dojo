# Leçon 11 — Interfaces et composition

> Dernière leçon du niveau 1, et la plus « culturelle » : c'est ici que le style Go se
> distingue vraiment de Java, C# ou Python.
...
type ReadWriter interface {
	io.Reader     // composition d'interfaces
	io.Writer
}
```

`io.ReadWriter`, `io.ReadCloser`, `io.ReadWriteCloser` sont construits ainsi. On compose de
petites interfaces plutôt que d'en écrire de grosses — c'est exactement la philosophie
générale de Go.

Une struct peut aussi embarquer une **interface**, technique utile pour n'implémenter qu'une
partie d'un contrat (fréquent dans les tests) :

```go
type partialStore struct {
	Store             // interface embarquée, nil
	getFn func(string) (*User, error)
}
func (p partialStore) Get(id string) (*User, error) { return p.getFn(id) }
// Les autres méthodes existent mais paniqueraient si appelées — souvent voulu en test.
```

### Composition plutôt qu'héritage : la synthèse

Go n'a ni classes, ni héritage, ni polymorphisme d'implémentation. Il a :
- l'**embedding de structs** pour réutiliser du code (leçon 10) ;
- les **interfaces** pour le polymorphisme, sans lien de parenté entre les types ;
- des **fonctions de première classe** pour le comportement paramétrable (leçon 5).

Ces trois outils couvrent l'essentiel de ce que fait l'héritage, sans hiérarchies fragiles.
La contrepartie honnête : pas de réutilisation d'implémentation « gratuite » sur trois
niveaux de classes — il faut écrire un peu de code de délégation. La plupart des
développeurs Go considèrent que c'est un excellent échange.
...
| `errors.Join(errs...)` | Go 1.20+ : agrège des erreurs, retourne `nil` si le slice est vide. Réponse à une question posée en leçon 8. |
...
9. **Mélanger récepteurs valeur et pointeur** : seul `*T` satisfait alors l'interface, et le
   ...
   la règle de cohérence de la leçon 10.
...
- L'embedding compose ; il ne remplace pas le polymorphisme, qui passe par les interfaces.
...
Prochaines étapes :
1. **Évaluation cumulative** du niveau 1 (demandée au professeur).
2. **[Projet 1 — `wordstat`](../../projets/projet-01-cli-simple/)**.
3. Puis le [niveau 2 — Go idiomatique](../../niveau-02-go-idiomatique/).
